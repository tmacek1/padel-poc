...
export default function MatchDetailPage() {
  ...
  const generateWhatsAppLink = () => {
    ...
    const dateStr = date.toLocaleDateString('hr-HR', { weekday: 'short', day: 'numeric', month: 'short' })
    const timeStr = date.toLocaleTimeString('hr-HR', { hour: '2-digit', minute: '2-digit' })
    const location = match.location?.name || 'TBD'

    let msg = `🎾 *PadelHub - Match*\n📅 ${dateStr}, ${timeStr}\n📍 ${location}\n`

    const rotationSchedule = getParsedRotationSchedule()
    if (rotationSchedule) {
      msg += `\n🔄 *Raspored parova:*\n`
      rotationSchedule.forEach(s => {
        const t1 = s.team1.map(p => p.name.split(' ')[0]).join(' / ')
        const t2 = s.team2.map(p => p.name.split(' ')[0]).join(' / ')
        msg += `Set ${s.setNumber}: ${t1} vs ${t2}\n`
      })
    } else {
      const t1 = getTeamPlayers(1)
      const t2 = getTeamPlayers(2)
      if (t1 && t2) msg += `\n${t1} vs ${t2}\n`
    }
        ...
        <div className="mt-6 flex items-center gap-4">
          ...
          {match.status === 'scheduled' && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition font-medium text-sm"
            >
